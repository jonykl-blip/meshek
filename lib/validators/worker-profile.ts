import { z } from "zod";

export const workerProfileBaseSchema = z.object({
  full_name: z
    .string()
    .min(1, "שם הוא שדה חובה")
    .max(100),
  email: z
    .string()
    .email("כתובת אימייל לא תקינה")
    .optional()
    .or(z.literal("")),
  telegram_id: z
    .string()
    .regex(/^\d+$/, "מזהה טלגרם חייב להיות מספרי")
    .optional()
    .or(z.literal("")),
  hourly_rate: z
    .number()
    .positive("תעריף חייב להיות חיובי")
    .multipleOf(0.01)
    .optional()
    .nullable(),
  language_pref: z.enum(["he", "th", "en"]),
  role: z.enum(["worker", "manager", "admin"]),
});

export const workerProfileSchema = workerProfileBaseSchema.superRefine((data, ctx) => {
  if (
    (data.role === "admin" || data.role === "manager") &&
    (!data.email || data.email.trim() === "")
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "אימייל הוא שדה חובה למנהלים",
      path: ["email"],
    });
  }
});

export const profileAliasSchema = z.object({
  alias: z
    .string()
    .min(1, "שם הכינוי הוא שדה חובה")
    .max(100),
});

export type WorkerProfileInput = z.infer<typeof workerProfileSchema>;
export type ProfileAliasInput = z.infer<typeof profileAliasSchema>;
